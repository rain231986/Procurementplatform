...
-- Receiving routing, per-store delivery plans and structured workflow blocks.
ALTER TABLE purchase_order_items
...
ALTER TABLE purchase_order_items ADD CONSTRAINT purchase_order_items_delivery_mode_check CHECK (delivery_mode IN ('SUPPLIER_DIRECT_TO_STORE', 'WAREHOUSE_DISTRIBUTION'));
...
UPDATE purchase_order_item_store_allocations SET destination_location_id = COALESCE(destination_location_id, location_id), warehouse_receipt_location_id = COALESCE(warehouse_receipt_location_id, (SELECT id FROM locations WHERE type = 'WAREHOUSE' ORDER BY id LIMIT 1)), planned_delivery_qty = CASE WHEN planned_delivery_qty = 0 THEN COALESCE(NULLIF(confirmed_allocation_qty, 0), suggested_allocation_qty) ELSE planned_delivery_qty END;
...
ALTER TABLE purchase_order_item_store_allocations ADD CONSTRAINT purchase_order_item_store_allocations_delivery_mode_check CHECK (delivery_mode IN ('SUPPLIER_DIRECT_TO_STORE', 'WAREHOUSE_DISTRIBUTION'));
...
ALTER TABLE purchase_order_item_store_allocations ADD CONSTRAINT purchase_order_item_store_allocations_status_check CHECK (status IN ('PLANNED', 'WAITING_SUPPLIER_SHIPMENT', 'WAITING_WAREHOUSE_RECEIPT', 'WAITING_STORE_DIRECT_RECEIPT', 'WAREHOUSE_RECEIVED', 'WAITING_WAREHOUSE_ALLOCATION', 'WAREHOUSE_SHIPPED', 'PARTIALLY_ALLOCATED', 'ALLOCATED', 'PARTIALLY_RECEIVED', 'RECEIVED', 'SHORT_RECEIVED', 'REJECTED', 'CANCELLED'));
...
CREATE TABLE IF NOT EXISTS supplier_direct_receipt_logs (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(), purchase_order_id uuid REFERENCES purchase_orders(id), purchase_order_item_id uuid REFERENCES purchase_order_items(id), store_allocation_id uuid REFERENCES purchase_order_item_store_allocations(id), destination_location_id uuid REFERENCES locations(id), operation_id varchar(160), signed_qty integer NOT NULL DEFAULT 0 CHECK (signed_qty >= 0), rejected_qty integer NOT NULL DEFAULT 0 CHECK (rejected_qty >= 0), signed_by uuid REFERENCES users(id), signed_at timestamptz NOT NULL DEFAULT now(), note text
);
...
CREATE TABLE IF NOT EXISTS workflow_notifications (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(), workflow_block_event_id uuid NOT NULL REFERENCES workflow_block_events(id) ON DELETE CASCADE, recipient_role varchar(32) NOT NULL, recipient_user_id uuid REFERENCES users(id), entity_id uuid NOT NULL, message text NOT NULL, is_read boolean NOT NULL DEFAULT false, read_at timestamptz, created_at timestamptz NOT NULL DEFAULT now()
);
