...
- `supplier-operations-workflow.js`：供應商付款/訂貨週期/銀行帳戶、商品國際代碼、採購明細追蹤與缺貨重新納池、供應商退貨與庫存 movement 的 clone-state transaction 邊界。
...
本次迭代新增 `supplier-operations-workflow.js`，但不改寫既有需求核單、自動補貨公式、總倉配貨/收貨核心或角色 enum。服務方法以明確 actor role 驗證：PURCHASING/ADMIN 維護供應商商務、訂購/付款關係、訂貨週期、銀行帳戶、國際代碼、逐品項追蹤與缺貨；WAREHOUSE/ADMIN 建立退貨、保留/出庫退貨及替代品入庫；STORE 只能透過投影取得自己的來源需求進度與供應商公開提示。

正式 API 需要將這些方法拆成 `supplier-master`、`supplier-return`、`purchase-item-follow-up` 與 `attachment` service，所有寫入以 database transaction 及 row lock 完成。付款對象與訂購對象會在採購單保存 snapshot；草稿切換付款對象、`ORDERED` 後任何例外切換都必須有原因與 audit。缺貨重新納池寫入新來源 relation，並保留每個來源需求的取消／重新採購數量，不能刪除或改寫原採購單；退貨草稿、準備退貨、正式出庫、退貨處理結果、替代品入庫各自有可追溯狀態。

目前 static Phase 1 使用 localStorage，因此附件只保存 metadata 與 private `storage_key`，沒有實際雲端 binary upload/download；正式部署必須由後端依 PURCHASING/WAREHOUSE/ADMIN 產生短效 authenticated signed download，STORE 不得收到 URL 或內容。`migrations/008_supplier_returns_shortage_tracking.sql` 與 `schema.sql` 已提供正式 PostgreSQL 表與索引契約。
