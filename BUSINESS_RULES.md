...
- 門市調撥由來源門市建立草稿，經來源門市店長核准後才可出貨；目的門市只能在收到實物後簽收。調撥狀態依序使用 `DRAFT`、`PENDING_SOURCE_APPROVAL`、`APPROVED`、`PARTIALLY_SHIPPED`、`SHIPPED`、`PARTIALLY_RECEIVED`、`RECEIVED`，退回、拒絕與取消均保留原因及歷程。
- 可調撥量使用 `max(0, on_hand_qty - reserved_qty - approved_unshipped_transfer_qty - safety_stock_qty)`。核准時再次檢查來源庫存、已保留需求、其他已核准未出貨調撥及安全庫存；一般門市不得因調撥低於安全庫存，ADMIN 可填寫原因覆核。核准不扣庫存，出貨才扣來源門市，目的門市簽收才增加庫存；每個 operation id 具冪等性。
- 調撥商品必須保存批號與效期（如商品需要批號／效期管理），並保存來源、目的、建立／核准／出貨／簽收人與時間。STORE 只能查看自己的門市作為來源或目的的資料，不能透過查詢參數取得其他門市。
- 安全庫存設定以門市／商品為單位，值必須是非負整數；可設定生效日、最大安全庫存與異動原因。設定為 0 表示不以安全庫存限制調撥。STORE 店長可修改自己門市，ADMIN 可修改全部門市，其餘角色只能依權限查看。
- 門市退回總倉須經店長核准與總倉核准，出貨時才扣門市庫存，總倉實際收貨時才增加總倉庫存；總倉拒收必須填原因，退回門市收貨後才可結案。門市直接退廠商須經店長、採購與廠商確認，出貨時扣門市庫存；廠商拒退須先回到門市，換貨須由門市實際收貨後才完成，不能僅靠採購或廠商回覆直接結案。
- 兩種退貨均保存退貨原因、批號／效期、附件 metadata、廠商回覆、預計處理日與每次處理歷程。STORE 不得查看採購內部備註或其他門市退貨資料；附件檔案仍依既有私有儲存與角色限制處理。
- 退貨出貨、拒收退回、換貨收貨及相關庫存異動必須在同一 transaction；任何資料庫或庫存更新失敗時，原狀態、庫存與歷程全部 rollback。

## 單一採購明細缺貨追蹤與共用商品表格

- 缺貨以採購單商品明細為單位，不得只以整張採購單表示。`shortage_qty` 不得超過該明細尚未收貨且未取消的數量；標記缺貨、暫時缺貨、長期缺貨、廠商欠貨或替代商品時必須填原因，`OTHER` 必須補充說明。
- 每次缺貨更新、取消缺貨、重新採購與指定替代商品都要寫入逐明細 follow-up history，保存操作者、時間、前後數量／狀態、原因、廠商回覆、預計日期與內部備註。門市投影只能顯示自己需求、公開狀態、回覆與日期，不得暴露採購內部備註。
- 缺貨關卡、調撥及退貨的阻擋錯誤使用 `WORKFLOW_BLOCKED` 與結構化 `blocking_items`；解除後保留事件歷史。所有新增資料的商品編號、商品名稱、規格、數量欄位套用共用表格換行規則，不以縮小字體或水平捲軸隱藏重要數量。
